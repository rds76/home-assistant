function baseiconsensor(widget_id, url, skin, parameters)
{
    ...
    {
        self.set_field(self, "title2", isNaN(state.state) ? state.state : self.format_number(self, state.state)
    ...
    {   
        ...
        {
            self.set_field(self, "state_text", (isNaN(value) ? value : self.format_number(self, value)) 
                   + (("sub_units" in self.parameters && self.parameters.sub_units != "") ? ' ' + self.parameters.sub_units :
                       ((state.attributes['unit_of_measurement'] ?  ' ' + state.attributes.unit_of_measurement : '')))
             );
        }
    }
